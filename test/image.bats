...
@test "container imageRef uses same repository as user-requested image" {
	start_crio

	# Create container using tagged reference
	jq '.image.image = "quay.io/crio/fedora-crio-ci:latest"' \
		"$TESTDATA"/container_config.json > "$TESTDIR"/ctr.json
	...
	# Verify imageRef contains a digest reference from the same repository
	[[ "$imageRef" == "quay.io/crio/fedora-crio-ci@sha256:"* ]]
}

@test "imageRef matches user-requested repository for fedora-crio-ci and fedora-crio-ci-mirror" {
	start_crio
